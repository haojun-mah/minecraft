...
_STAGES_PHASE1: list[tuple[str, float, float]] = [
    ("research", 0.10, 0.4),
    ("planning", 0.25, 0.4),
    ("image_gen", 0.45, 0.6),
...
async def run_pipeline(
    ...
    try:
        ...
        await _simulate_pipeline(job_id, store)
        result = build_sample_response(job_id=job_id, prompt=request.prompt)
...
async def _simulate_pipeline(job_id: str, store: JobStore) -> None:
    """Phase-1 placeholder: walk through the same stages the real pipeline will."""
    for stage, progress, delay_s in _STAGES_PHASE1:
